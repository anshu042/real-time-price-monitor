...
/**
 * Escape text before interpolating it into the email HTML. Product names and
 * URLs originate from scraped third-party pages and are therefore untrusted.
 */
...
/** Only allow http(s) links in the email body. */
...
export async function sendPriceDropAlert(
  ...
  try {
    ...
    const currency = escapeHtml(product.currency);
    const productUrl = safeUrl(product.url);
    const imageUrl = product.image_url ? safeUrl(product.image_url) : null;
    const appUrl = safeUrl(process.env.NEXT_PUBLIC_APP_URL || "");
            ...
            <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
              ...
              ${
                ...
                <div style="text-align: center; margin-bottom: 20px;">
                  <img src="${imageUrl}" alt="${name}" style="max-width: 200px; height: auto; border-radius: 8px; border: 1px solid #e5e7eb;">
                </div>
              `
                  : ""
              }
              
              <h2 style="color: #1f2937; margin-top: 0;">${name}</h2>
              ...
              <div style="border-top: 1px solid #e5e7eb; padding-top: 20px; margin-top: 20px; text-align: center; color: #6b7280; font-size: 12px;">
                ...
                <p style="margin-top: 10px;">
                  <a href="${appUrl}" style="color: #FA5D19; text-decoration: none;">
